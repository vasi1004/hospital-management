...
from app.schedule.services.codes import next_doctor_code
...
def _ensure_unique_auth_user(
    db: DbSession, auth_user_id: int, *, exclude_doctor_id: int | None = None
) -> None:
    query = db.query(Doctor).filter(Doctor.auth_user_id == auth_user_id)
    if exclude_doctor_id is not None:
        query = query.filter(Doctor.id != exclude_doctor_id)
    if query.first():
        raise HTTPException(
            status_code=409,
            detail="This login account is already linked to another doctor profile",
        )
...
def create_doctor(payload: DoctorCreate, db: DbSession, _: AdminOnly) -> DoctorPublic:
    ...
    _ensure_unique_auth_user(db, payload.auth_user_id)

    # Availability is doctor-owned; new profiles start with no bookable hours.
    doctor = Doctor(
...
def update_doctor(
    ...
    _ensure_unique_auth_user(
        db, payload.auth_user_id, exclude_doctor_id=doctor.id
    )

    # Never overwrite available_* — only the linked doctor may set those.
