...
from app.schedule.services.codes import next_patient_code
...
def create_patient(
    ...
    patient = Patient(patient_code=next_patient_code(db), **payload.model_dump())
    db.add(patient)
    db.commit()
    db.refresh(patient)
    return _to_public(patient)
...
def deactivate_patient(patient_id: int, db: DbSession, _: MutateRoles) -> PatientPublic:
    patient = db.query(Patient).filter(Patient.id == patient_id).first()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
